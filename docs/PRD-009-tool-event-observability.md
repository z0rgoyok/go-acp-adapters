...
`kind` является SDK‑совместимой категорией инструмента. Если Claude tool name
не удаётся безопасно сопоставить SDK enum, поле опускается.
...
Если `is_error = true`, status становится `failed`, а в `_meta` добавляется
`isError: true`.
...
- `toolCallId`;
- `status`;
- `content` (массив `ToolCallContent`, совместимый с ACP SDK).

В режиме `full` дополнительно заполняется `rawOutput` с оригинальным JSON
...
остался совместимым, а tool updates соответствовали `github.com/coder/acp-go-sdk
v0.13.0`.

Минимальная SDK‑совместимая shape:
...
`ToolKind` — SDK‑совместимый enum: `read`, `edit`, `delete`, `move`, `search`,
`execute`, `think`, `fetch`, `switch_mode`, `other`.

`ToolCallStatus` — `pending`, `in_progress`, `completed`, `failed`.

`Content` для tool result — JSON‑массив `ToolCallContent` с элементами вида
`{"type":"content","content":{"type":"text","text":"..."}}`. Для text update
`Content` остаётся одиночным `ContentBlock`.
...
- update получает `_meta` с полями `truncated` и `originalBytes`;
...
- `tool_result` transcript event отправляется как `session/update` с
  `sessionUpdate = "tool_call_update"`.
- Tool call update содержит `toolCallId`, `kind` (SDK‑совместимый), `status=pending`,
  `rawInput`.
- Tool result update содержит `toolCallId`, `status`, `content` (массив
  `ToolCallContent`), diagnostic `_meta`.
- `is_error = true` мапится в `status = "failed"` и `_meta.isError = true`.
...
- `--tool-events full` отправляет полный input/result в пределах hard limit,
  `rawOutput` заполняется оригинальным JSON.
...
- SDK contract tests проверяют decode всех variants через
